...
export function createRateLimiter(options = {}) {
  const limit = getPositiveInteger(options.limit, DEFAULT_RATE_LIMIT)
  const windowMs = getPositiveInteger(options.windowMs, DEFAULT_RATE_LIMIT_WINDOW_MS)
  ...
  return (clientId) => {
    ...
    if (!client || currentTime >= client.resetAt) {
      if (!client && clients.size >= MAX_RATE_LIMIT_CLIENTS) {
    ...
    if (client.count >= limit) {
      return Math.max(1, Math.ceil((client.resetAt - currentTime) / 1000))
    }
...
export async function handleFrameCheckRequest(request, response, options = {}) {
  try {
    ...
    const retryAfter = (options.rateLimiter || defaultRateLimiter)(getClientId(request))
    if (retryAfter) {
      throw new PublicError(
        429,
        'FRAME_CHECK_RATE_LIMITED',
        'Too many frame checks. Please wait before trying again.',
...
const defaultRateLimiter = createRateLimiter({
  limit: process.env.FRAME_CHECK_RATE_LIMIT,
  windowMs: process.env.FRAME_CHECK_RATE_LIMIT_WINDOW_MS,
})
